class Connector {
     ...
     * @param {Function} callback Called when the request is done.
     * **/
    makeToNewReferenceImage (id, callback) {
     ...
     * @param {Function} callback Called when the request is done.
     * **/
    modifyIgnoreAreas (id, ignoreAreas, callback) {
     ...
     * @param {Function} callback Called when the request is done.
     * **/
    modifyCheckAreas (id, checkAreas, callback) {
     ...
     */
    editProject (newName, id, callback) {
     ...
     * @param {Function} callback callback Called when the request is done.
     */
    assignImageSetToProject (imageSetId, projectIdFrom, projectIdTo, callback) {
    ...
    sendRequest (url, method, data = {}, callback) {
        data = {};
        let serverEndpoint = location.protocol
            + '//' + location.hostname + (location.port ? ':' + location.port : '') + '/api' + url;
        console.log('url', serverEndpoint);
        console.log('data', data);
        window.fetch(serverEndpoint, {
            method: method,
            mode: 'cors',
            cache: 'no-cache',
            headers: {
            ...
            .catch( error => {
                console.error('Request failure: ', error, url, method, data);
